...
| 3 | `cargo audit` | 3–5s | RustSec advisory DB against `Cargo.lock` |
| 4 | `cargo test --release --all-targets` | 10–90s | unit + integration tests (lib + `tests/*.rs`) |
| 5 | `tests/live/test_run.sh` | ~30 s | 88 read-only probes against the live cluster |
| 6 | `tests/live/test_mutation.sh` | ~60 s | full mutation lifecycle: LXC 9999 + cluster-level CRUD + QEMU 9998 + opt-in QGA |

Total cold: ~3–4 min. Total warm (cargo cache hot): ~75s.
