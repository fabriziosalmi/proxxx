...
# ── Phase 11: 2026-05-04 PVE API expansion — read-only probes ──
# Each region has a corresponding wiremock-only row in
# pre-commit/01-feature-coverage.md; these probes flip the read side
# of those rows from ⚠️ wiremock-only to ✅ live-verified. Mutation
# lifecycle (CRUD round-trips) lives in test_mutation.sh, added in
# follow-up batches.
