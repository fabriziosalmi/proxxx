...
| :--- | :--- | :---: |
| Secret heap residue | `Zeroizing<String>` everywhere | V16 |
...
| API body OOM | 32 MiB body cap on every response | V14 |
| Async deadlocks | `await_holding_lock = "deny"` | clippy |
| Supply chain | `cargo audit --deny warnings` in gate + CI | V19 |
| Panic recovery | Flight recorder hook + RAII terminal restore | V26.8 |
| Daemon shutdown | SIGTERM handler with WAL flush | V21 |
...
## Zeroizing&lt;String&gt; everywhere (V16)
...
## Panic recovery (V26.8 + flight recorder)
