...
> **Status**: living document, last consolidated 2026-05-19. Anything
...
| Surface | Mitigation | Pinned at |
| :--- | :--- | :--- |
| TLS | rustls only (no native-tls / openssl in the tree — banned in `deny.toml`); `verify_tls = true` by default; per-profile TOFU pinning available (`tls_pin_mode = "tofu"`) | [src/api/client.rs](src/api/client.rs), [src/api/tls_pin.rs](src/api/tls_pin.rs) |
| Auth | Token bearer via OS keychain (`keyring` crate) OR `Zeroizing<String>` env var. Cookies + CSRF token round-trip for password auth (legacy) | [src/api/auth.rs](src/api/auth.rs) |
...
| WebSocket (`/vncwebsocket?vncticket=...`) | 1 MiB max frame / 4 MiB max message ceilings (explicit, not relying on tungstenite defaults). Custom rustls verifier when `verify_tls = false`. Backpressure: synchronous stdout write between frames | [src/wsterm/mod.rs](src/wsterm/mod.rs) |
...
| HITL gating on destructive tools | The same destructive-op list that the CLI uses gates MCP — `delete_guest`, `stop_guest`, `restart_guest`, `migrate_guest` go through the same HITL approval channel | [src/mcp/dispatch.rs](src/mcp/dispatch.rs) |
| Oversize-line DoS (stdio) | 16 MiB hard cap on a single JSON-RPC line; drain-and-continue rather than allocate-and-crash | [src/mcp/server.rs:18](src/mcp/server.rs#L18) `MAX_RPC_LINE_BYTES` |
...
| Surface | Mitigation | Pinned at |
| :--- | :--- | :--- |
| Audit log tampering | HMAC-SHA256 chain per entry: `chain_hmac = HMAC(key, prev || ts || action || vmid || result)`. `proxxx audit verify` recomputes the chain and reports first broken link. 4 proptests pin: round-trip; any single-column mutation breaks ≥ 1 link; mutation blast radius is exactly 1 or 2 rows (pinpoint over cascade — deliberate design); out-of-chain columns (`user`, `node`, `params_json`) leave chain intact | [src/audit/mod.rs](src/audit/mod.rs) `mod proptests` |
...
| `verify_tls = false` profiles | Operator opt-in for homelab self-signed certs; the TUI tag-shows `INSECURE` on any view of that profile. Pin via `tls_pin_mode = "tofu"` is the preferred path | [src/config/mod.rs](src/config/mod.rs) |
...
| Test | `cargo test --all-targets` | 380+ tests (lib + 17 integration files + RBAC live) |
