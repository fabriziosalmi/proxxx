...
<p align="center">
  <img src="assets/demo-mcp.svg" alt="proxxx MCP server — `proxxx mcp tools` reports 25 typed tools (10 flagged destructive); an MCP tools/call list_guests returns live guest data to the agent" width="820">
  <br>
...
| [`src/state/`](src/state) | GitOps loop — model + export + diff + apply + preflight, ten state families (pools, ACL, storage, backup-jobs, firewall-cluster, notifications, HA rules, HA resources, PCI/USB mappings), byte-stable TOML. |
...
| [`test_mutation.sh`](tests/live/test_mutation.sh) | ✓ | 34 mutation probes with `trap EXIT` cleanup: LXC 9999 (create → start → snapshot → stop → delete), cluster-level CRUD across 8 of the 10 state families (pool / ACL / storage-defs / backup-job / firewall-cluster alias+group+ipset / notifications matcher / HA rules / HA resources — PCI/USB passthrough mappings not yet in the harness), QEMU 9998 from alpine ISO, opt-in QGA round-trips via `PROXXX_E2E_QGA_VMID=<vmid>` |
