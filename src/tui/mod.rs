...
enum DataMsg {
    ...
    /// Vector 17: live cluster-quorum status. `false` means proxxx is
...
impl HitlCoordinator {
    ...
    async fn resolve(&self, txn_id: &str, approved: bool) {
        if let Some(sender) = self.pending.lock().await.remove(txn_id) {
...
pub async fn run(profile: Option<&str>, cli_secret: Option<&str>, secure: bool) -> Result<()> {
    ...
    let tg_gateway: Option<Arc<crate::hitl::telegram::TelegramGateway>> = client
        .profile_config()
        .telegram
        .clone()
        .map(|cfg| Arc::new(crate::hitl::telegram::TelegramGateway::new(cfg)));
    ...
    // Vector 8 (Gemini audit) — resize debounce. While the user
    ...
    loop {
        // Vector 8: flush a debounced PTY resize once the drag has
                ...
                if let Some(evt) = evt {
                    match evt {
                        event::AppEvent::Key(key) => {
                            // Vector 13 (Gemini wave-3 audit) — Ctrl+L force-redraw.
                            //
                        ...
                        event::AppEvent::Resize(w, h) => {
                            // Vector 8: stage the latest dimensions and
    ...
    // ── Terminal Teardown ────────────────────────────────
    // V26.8: explicit restore on the happy path so we surface IO
...
async fn fetch_all(client: &Arc<PxClient>, tx: &mpsc::Sender<DataMsg>) {
    ...
    match client.get_nodes().await {
        Ok(nodes) => {
            ...
            // Vector 17 (macro audit) — cluster quorum sanity check.
            //
...
async fn dispatch_side_effect(
    ...
    let check_hitl = |action: &str, vmid: u32, effect: SideEffect| -> bool {
        ...
        if policy_match.is_some() || secure_required {
            ...
            let desc = format!(
                "Operation {} on {} requires approval via {}",
                action, vmid, channel
            );
            ...
            tokio::spawn(async move {
                ...
                let approved = match tokio::time::timeout(Duration::from_secs(120), receiver).await
                {
                    Ok(Ok(b)) => b,
                    _ => {
    ...
    match effect {
        ...
        SideEffect::MigrateGuest {
            ...
            if let Some(gt) = gt {
                ...
                tokio::spawn(async move {
                    match client_cloned
                        .migrate_guest(&node, vmid, gt, &target_node)
                        .await
                    {
                        Ok(upid) => {
                            let _ = tx_cloned.send(DataMsg::TaskStarted(upid)).await;
        ...
        SideEffect::ExecuteGuestCommand {
            ...
            tokio::spawn(async move {
                ...
                {
                    Ok(res) => {
                        info!("Guest {vmid} command success: {res}");
                    }
                    Err(e) => {
                        ...
                        if let Some(node) = target_node {
                            ...
                            let res = match action {
                                ...
                                app::Action::MigrateGuest { target_node, .. } => {
                                    client_cloned
                                        .migrate_guest(&node, vmid, g_type, &target_node)
                                        .await
                                }
        ...
        SideEffect::FetchHaConsoleData => {
            ...
            let nodes_snapshot: Vec<String> = state
                ...
                .collect();
            tokio::spawn(async move {
                // Vector 5 (Gemini audit) — explicit per-call timeouts.
                //
...
async fn run_demo() -> Result<()> {
    // V26.8 — same TerminalGuard treatment as `run()`.
    let mut term_guard = TerminalGuard::install()?;
    let terminal = term_guard.terminal_mut();
    terminal.clear()?;
...
fn draw(f: &mut Frame, state: &AppState, ssh: &ssh_handler::SshSessionHandler) {
    ...
    // Vector 17 — render the QUORUM LOST banner whenever the most
