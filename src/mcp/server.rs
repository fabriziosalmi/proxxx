...
/// Vector 10 (Gemini audit) — hard cap on a single JSON-RPC line.
///
...
pub async fn run_server(client: Arc<PxClient>, config: Arc<ProfileConfig>) -> Result<()> {
    ...
    loop {
        line_buf.clear();
        // Vector 10: read at most MAX_RPC_LINE_BYTES via the `take`
        ...
        if truncated {
            ...
            send_error(
                ...
                -32700,
                &format!(
                    "Parse error: request exceeds {} byte limit",
                    MAX_RPC_LINE_BYTES
                ),
            )
            .await?;
        ...
        if let Some(guest_id) = args.get("guest_id").and_then(serde_json::Value::as_u64) {
            ...
            if let Some(policy) = crate::hitl::policy::check_policies(
                ...
                if let Some(ref tg) = config.telegram {
                    let tg_gateway = crate::hitl::telegram::TelegramGateway::new(tg.clone());
