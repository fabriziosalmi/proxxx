...
fn backoff_delay(attempt: u32, retry_after_secs: Option<u64>) -> std::time::Duration {
    ...
    // Cheap jitter: process-scoped nanosecond fraction. Adds 0..base/4 ms.
...
/// Vector 14 (Gemini wave-3 audit) — hard cap on response body size
...
/// V26.3 (audit) — threshold above which `serde_json::from_slice` is
...
/// so the reactor stays responsive — V26.3 audit fix.
async fn parse_json_maybe_blocking<T>(bytes: Vec<u8>, path: &str) -> Result<T>
where
...
async fn read_bounded_body(resp: Response, path: &str) -> Result<Vec<u8>> {
    use futures_util::StreamExt;

    if let Some(declared) = resp.content_length() {
        if declared as usize > MAX_RESPONSE_BYTES {
            anyhow::bail!(
                "response from {path} declares {} bytes, exceeds {MAX_RESPONSE_BYTES} limit",
                declared
            );
        }
    }
...
fn default_true_int_local() -> bool {
    true
}
...
impl PxClient {
    ...
    pub async fn new(config: ProfileConfig, cli_secret: Option<&str>) -> Result<Self> {
        let http = Client::builder()
            ...
            // V29.1 (audit) — TCP keepalive on the reqwest pool.
            //
        ...
        loop {
            ...
            match result {
                Ok(resp) => {
                    ...
                    // Vector 11 (Gemini wave-3 audit) — reactive re-auth.
                    //
    ...
    /// Vector 11: bypass the proactive `needs_refresh()` check and
    ...
    async fn get<T: DeserializeOwned + Send + 'static>(&self, path: &str) -> Result<T> {
        ...
        if !status.is_success() {
            ...
            // V26.4: typed ApiError so the TUI/CLI can downcast and
        ...
        // Vector 14: read the body into a bounded buffer before
        ...
        // V26.3: route large parses through spawn_blocking so a
...
impl ProxmoxGateway for PxClient {
    ...
    async fn stop_guest(
        ...
        let kind = type_path(guest_type);
        let mut params = vec![];
        // `forceStop=1` is QEMU-only — it tells PVE to also SIGKILL the qemu
        // process if the soft stop hangs. The LXC stop endpoint rejects it
        // with a 400 schema violation; LXC stop is already a hard kill of
        // the container init, so there's no soft/force distinction.
        if force && matches!(guest_type, GuestType::Qemu) {
            params.push(("forceStop", "1"));
        }
        let resp: ApiResponse<String> = self
            .post(&format!("/nodes/{node}/{kind}/{vmid}/status/stop"), &params)
            .await?;
        Ok(resp.data)
    }
    ...
    async fn migrate_guest(
        ...
        let kind = type_path(guest_type);
        let params = vec![("target", target_node)];
        let resp: ApiResponse<String> = self
    ...
    async fn execute_guest_command(
        ...
        // V27.5 (audit) — QEMU agent exec has its own failure mode:
        ...
        match guest_type {
            crate::api::types::GuestType::Qemu => {
                ...
                let resp = tokio::time::timeout(
                    QGA_EXEC_TIMEOUT,
                    self.post::<ApiResponse<crate::api::types::AgentExecResponse>>(&path, &params),
                )
                .await
                .map_err(|_| {
                    anyhow::anyhow!(
                        "QEMU guest agent on guest {vmid} did not respond within {:?} \
                         — agent likely wedged (kernel hang, Windows update, or QGA not installed)",
                        QGA_EXEC_TIMEOUT
                    )
                })??;
                Ok(format!("PID: {}", resp.data.pid))
            }
            crate::api::types::GuestType::Lxc => {
                // LXC `exec` goes through `pveproxy` directly, not the
                // guest agent — its hang profile is different (host-
                // side `lxc-attach`). Same timeout for symmetry; the
                // LXC equivalent of a wedged agent is a forked-but-
                // dead init process.
                let path = format!("/nodes/{node}/lxc/{vmid}/exec");
                let resp = tokio::time::timeout(
                    QGA_EXEC_TIMEOUT,
                    self.post::<ApiResponse<String>>(&path, &params),
                )
                .await
                .map_err(|_| {
                    anyhow::anyhow!(
                        "LXC exec on guest {vmid} did not respond within {:?} \
                         — container init may be wedged",
                        QGA_EXEC_TIMEOUT
                    )
                })??;
                Ok(resp.data)
    ...
    async fn download_to_storage(
        ...
        // BLOCKER 1 schema: caller passes (algo, hex) together. Both
    ...
    async fn move_disk(
        ...
        // V27.10 (audit) — storage backends behave very differently:
    ...
    async fn resize_disk(
        ...
        if !status.is_success() {
            let body = resp.text().await.unwrap_or_default();
            anyhow::bail!("PUT resize {vmid} returned {status}: {body}");
        }
