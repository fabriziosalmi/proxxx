...
/// Vector 12 (Gemini wave-3 audit) — keychain access wrapper.
///
...
/// Vector 26.10 (audit) — bounded env var read for secrets.
///
...
impl PbsConfig {
    ...
    /// `spawn_blocking` (Vector 12 audit) — see `keyring_get`.
...
impl TelegramConfig {
    ...
    /// (Vector 12 audit) — keyring calls block on Linux.
    ///
    /// Returns `Zeroizing<String>` so the heap bytes are wiped on
    /// drop, matching the PVE/PBS pattern (Vector 16 audit).
...
/// SSH layer config (Pillar 0). Optional per-profile.
///
...
impl ProfileConfig {
    ...
    pub async fn resolve_token_secret(
        ...
        // 2. Environment variable (V26.10 — 64 KiB cap)
        ...
        // 5. OS Keychain (if feature enabled) — Vector 12: blocking
    ...
    pub async fn resolve_password(&self) -> Result<zeroize::Zeroizing<String>> {
        ...
        // V26.10 — bounded env var read for the PVE password.
