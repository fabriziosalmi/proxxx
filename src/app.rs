...
pub struct AppState {
    ...
    /// Vector 17 (macro audit): cluster quorum status from the most
    ...
    /// V27.6 (audit) — names of nodes whose `uptime` did not
...
pub enum Action {
    ...
    /// Vector 17: live cluster quorum flag — drives the TUI banner.
    ...
    /// or `("sha512", "...")`. BLOCKER 1 schema (was `sha256: Option<String>`).
    DownloadIsoCustom {
        url: String,
        filename: String,
        node: String,
...
impl AppState {
    ...
    ///
    /// V26.7 (audit) — release view-scoped state when its owner view
...
/// V27.1 + V27.4 (audit) — return Some(reason) if `vmid` cannot
...
pub fn update(state: &mut AppState, action: Action) -> Option<SideEffect> {
    match action {
        ...
        Action::NodesLoaded(nodes) => {
            // V27.6 (audit) — detect frozen pvestatd. PVE marks a
        ...
        Action::StopGuest { vmid, force } => {
            // V27.1 + V27.4 (audit) — refuse destructive ops when:
            ...
            if let Some(reason) = guest_block_reason(state, vmid, "stop") {
        ...
        Action::RestartGuest { vmid } => {
            // V27.1 + V27.4: same gate as StopGuest. Restart on a
            // locked / HA-managed guest is equally problematic.
            if let Some(reason) = guest_block_reason(state, vmid, "restart") {
                state.error = Some(reason);
                state.error_time = Some(std::time::Instant::now());
                return None;
            }
        ...
        Action::MigrateGuest { vmid, target_node } => {
            // V27.4 — only the lock check applies here. HA-managed
            // guests CAN be migrated (the CRM allows it), but a
            // backup/clone/snapshot lock conflicts with the move.
            if let Some(g) = state.guests.iter().find(|g| g.vmid == vmid) {
                if g.is_locked() {
        ...
        Action::DeleteGuest { vmid } => {
            // V27.1 + V27.4: HA-managed or locked → refuse loudly.
            // The API-level TOCTOU pre-flight (V2.3) is the last
            // line of defence; this is the user-visible early gate.
            if let Some(reason) = guest_block_reason(state, vmid, "delete") {
                state.error = Some(reason);
                state.error_time = Some(std::time::Instant::now());
                return None;
            }
        ...
        Action::CreateSnapshot { vmid, name } => {
            // V27.4: snapshot creation collides with backup/clone
            // locks — refuse early.
        ...
        Action::DownloadIso {
            ...
            // BLOCKER 1 gate: refuse curated-library downloads when the
        ...
        Action::MoveDisk {
            ...
            let queued = queue::QueuedOp::new(Box::new(a), state);
...
pub enum SideEffect {
    ...
    /// (never by direct Action::MoveDisk). See feature #6 in features.md.
    MoveDisk {
        node: String,
        vmid: u32,
        guest_type: crate::api::types::GuestType,
...
/// path below.
fn parse_command(_cmd: &str) -> Option<SideEffect> {
