...
/// Vector 18 (macro audit) — current schema version.
///
...
fn open_db(path: &Path) -> anyhow::Result<Connection> {
    ...
    // V28.3 — `auto_vacuum` is sticky: persisted in the DB header,
    ...
    // Vector 9 (Gemini audit) — bound WAL growth.
    //
    ...
    // Vector 18 — schema versioning + migration. Run BEFORE any DDL
...
pub fn save_state(
    ...
    // V28.3 — reclaim freelist pages back to the OS. Without this
...
mod concurrency_tests {
    ...
    ///
    /// Vector 18 (macro audit) regression: `open_db` MUST set
    ...
    /// V28.3 (audit) regression — `auto_vacuum` MUST be set to
    ...
    /// Vector 18 — refuse to open a DB written by a NEWER proxxx.
    ...
    /// Vector 9 (Gemini audit) extension: also assert
