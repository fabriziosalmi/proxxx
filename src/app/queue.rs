...
pub struct QueuedOp {
    ...
    /// Break-glass override: when true, the queue dispatcher SKIPS
    /// the pre-flight risk assessment for this op. Set only by the
    /// `Action::ConfirmForce` path (user explicitly pressed `F` on
    /// the destructive Confirm modal). Audit-logged on use.
    ///
    /// Rationale: HITL/queue gates without an emergency override are
    /// a trap — when Telegram is down or the cluster is on fire, the
    /// admin needs a local break-glass. The override never bypasses
    /// PVE's own server-side guards (lock state, HA-managed) — those
    /// still fail loudly at the API layer.
...
impl QueuedOp {
    ...
    /// Companion constructor for the break-glass path. Only the
    /// `Action::ConfirmForce` reducer should call this; everything
    /// else should use `new()` so preflight protections apply.
    #[must_use]
    pub fn new_force(action: Box<Action>, state: &AppState) -> Self {
        let mut q = Self::new(action, state);
        q.bypass_preflight = true;
        q
    }
    ...
    pub fn from_persisted(entry: PersistedQueueEntry) -> Self {
        ...
        Self {
            ...
            // Persisted ops always re-load with bypass_preflight=false.
            // The override is intentionally in-memory only — a TUI
            // restart MUST re-prompt the user to break the glass again.
            // Otherwise a force flag could survive across reboots and
            // bypass safety silently. Audit-friendly default.
            bypass_preflight: false,
        }
    }
}
...
pub fn garbage_collect(queue: &mut Vec<QueuedOp>, now_secs: u64) -> usize {
    ...
    if error_count > ERROR_KEEP {
        ...
        // chronologically ordered by enqueue time). Collecting into a
        // Vec is REQUIRED here — the borrow on `queue.iter()` would
        // conflict with the `queue.remove(i)` call below, so clippy's
        // `needless_collect` is a false positive in this pattern.
    ...
    if queue.len() > HARD_CAP {
        ...
        // Same borrow-conflict reasoning as Pass 2 above.
        #[allow(clippy::needless_collect)]
        let victim_indices: Vec<usize> = queue
            .iter()
            .enumerate()
