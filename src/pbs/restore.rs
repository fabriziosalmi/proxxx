    ...
    F: FnMut(&str) + Send,
{
    ...
    // Vector 16 — `cmd.env(...)` clones into the Command's internal
    ...
    cmd.arg("restore")
        ...
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        // BLOCKER 2: if the parent dies abnormally (panic, OOM, etc.),
    ...
    loop {
        // BLOCKER 2: graceful Ctrl+C handling. `tokio::signal::ctrl_c()`
...
mod tests {
    ...
    // BLOCKER 2: kill_on_drop smoke test. Spawn `sleep` directly via
