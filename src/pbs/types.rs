...
impl ArchiveInfo {
    ...
    ///
    /// Case-sensitive comparison is intentional: PBS server-generates
    /// these filenames in lowercase per protocol; an upper-case
    /// `.PXAR` would be a malformed/spoofed payload we don't want to
    /// silently accept as a directory archive.
    #[must_use]
